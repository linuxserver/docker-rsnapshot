pipeline {
  ...
  environment {
    ...
    BUILD_VERSION_ARG = 'RSNAPSHOT_VERSION'
    LS_USER = 'linuxserver'
    LS_REPO = 'docker-rsnapshot'
    CONTAINER_NAME = 'rsnapshot'
    DOCKERHUB_IMAGE = 'linuxserver/rsnapshot'
    DEV_DOCKERHUB_IMAGE = 'lsiodev/rsnapshot'
    PR_DOCKERHUB_IMAGE = 'lspipepr/rsnapshot'
    DIST_IMAGE = 'alpine'
    DIST_TAG = '3.12'
    DIST_REPO = 'http://nl.alpinelinux.org/alpine/v3.12/main/'
  ...
  stages {
    ...
    stage("Scarf.sh package registry"){
      ...
      steps{
        sh '''#! /bin/bash
              set -e
              PACKAGE_UUID=$(curl -X GET -H "Authorization: Bearer ${SCARF_TOKEN}" https://scarf.sh/api/v1/packages | jq -r '.[] | select(.name=="linuxserver/rsnapshot") | .uuid')
              if [ -z "${PACKAGE_UUID}" ]; then
                echo "Adding package to Scarf.sh"
                PACKAGE_UUID=$(curl -sX POST https://scarf.sh/api/v1/packages \
                  -H "Authorization: Bearer ${SCARF_TOKEN}" \
                  -H "Content-Type: application/json" \
                  -d '{"name":"linuxserver/rsnapshot",\
                       "shortDescription":"example description",\
                       "libraryType":"docker",\
                       "website":"https://github.com/linuxserver/docker-rsnapshot",\
                       "backendUrl":"https://ghcr.io/linuxserver/rsnapshot",\
                       "publicUrl":"https://lscr.io/linuxserver/rsnapshot"}' \
                  | jq -r .uuid)
              else
    ...
    stage('Build-Single') {
      ...
      steps {
        ...
        sh "docker build \
          ...
          --label \"org.opencontainers.image.url=https://github.com/linuxserver/docker-rsnapshot/packages\" \
          --label \"org.opencontainers.image.documentation=https://docs.linuxserver.io/images/docker-rsnapshot\" \
          --label \"org.opencontainers.image.source=https://github.com/linuxserver/docker-rsnapshot\" \
          --label \"org.opencontainers.image.version=${EXT_RELEASE_CLEAN}-ls${LS_TAG_NUMBER}\" \
          --label \"org.opencontainers.image.revision=${COMMIT_SHA}\" \
          --label \"org.opencontainers.image.vendor=linuxserver.io\" \
          --label \"org.opencontainers.image.licenses=GPL-3.0-only\" \
          --label \"org.opencontainers.image.ref.name=${COMMIT_SHA}\" \
          --label \"org.opencontainers.image.title=Rsnapshot\" \
          --label \"org.opencontainers.image.description=[Rsnapshot](http://www.rsnapshot.org/) is a filesystem snapshot utility based on rsync. rsnapshot makes it easy to make periodic snapshots of local machines, and remote machines over ssh. The code makes extensive use of hard links whenever possible, to greatly reduce the disk space required.  \" \
